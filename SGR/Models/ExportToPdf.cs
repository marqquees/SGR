...
///     O ambiente de alojamento que fornece informações sobre o ambiente da aplicação Web,
///     como a raiz do conteúdo e os caminhos da raiz da Web.
/// </param>
...
public class ExportToPdf(IWebHostEnvironment environment, NavigationManager navigationManager)
{
    ...
    /// </summary>
    /// <param name="selectedItem">O equipment de inventário a ser exportado para PDF.</param>
    ...
    public async Task<string> ExportEquipmentToPdfAndNavigate(Equipment? selectedItem)
    {
        try
        {
            if (selectedItem == null)
                return "Erro: equipment não selecionado.";

            // Gera o PDF e obtém o caminho do arquivo.
            var result = await ExportEquipmentToPdf(selectedItem);
